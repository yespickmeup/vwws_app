...
import com.vwwsapp.activities.LoginActivity;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String TABLE_OCCUPANCY_TYPES = "occupancy_types";
    ...
    private String CREATE_READINGS_TABLE = "CREATE TABLE " + TABLE_READINGS + "("
            ...
            + "pipe_size" + " varchar(255)"
            + ")";
    private String CREATE_OCCUPANCY_TYPES_TABLE = "CREATE TABLE " + TABLE_OCCUPANCY_TYPES + "("
            + "id" + " INTEGER PRIMARY KEY AUTOINCREMENT,"
            + "occupancy" + " varchar(255),"
            + "occupancy_type_id" + " varchar(255),"
            + "occupancy_type_name" + " varchar(255),"
            + "occupancy_type_code" + " varchar(255),"
            + "pipe_size" + " varchar(255),"
            + "cubic" + " varchar(255),"
            + "mf" + " double,"
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(CREATE_OCCUPANCY_TYPES_TABLE);
//        addSetting();
    }
...
//                values.put(COLUMN_1, reader.getId());
    ...
    public void addOccupancy(List<OccupancyTypes> occupancy) {
        SQLiteDatabase db = this.getWritableDatabase();

        try {

            db.execSQL("delete from "+ TABLE_OCCUPANCY_TYPES);
            for(OccupancyTypes occ: occupancy){
                ContentValues values = new ContentValues();
                values.put("occupancy", occ.getOccupancy());
                values.put("occupancy_type_id", occ.getOccupancy_type_id());
                values.put("occupancy_type_name", occ.getOccupancy_type_name());
                values.put("occupancy_type_code", occ.getOccupancy_type_code());
                values.put("pipe_size", occ.getPipe_size());
                values.put("cubic", occ.getCubic());
                values.put("mf", occ.getMf());
                values.put("mr", occ.getMr());
                values.put("charge",occ.getCharge());
                values.put("date_added", occ.getDate_added());
                values.put("date_updated", occ.getDate_updated());
                values.put("added_by_id", occ.getAdded_by_id());
                values.put("update_by_id", occ.getUpdate_by_id());
                values.put("status", occ.getStatus());
                values.put("remarks", occ.getRemarks());
                // Inserting Row
                db.insert(TABLE_OCCUPANCY_TYPES, null, values);
            }
    ...
    public void addReadingsOverride(List<Reading> readings) {
        SQLiteDatabase db = this.getWritableDatabase();

        try {

            db.execSQL("delete from readings where meter_reader_id=0 ");
